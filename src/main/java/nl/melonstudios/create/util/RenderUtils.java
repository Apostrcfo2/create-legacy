...
public class RenderUtils {
    ...
    public static void renderFluidCube(
            FluidStack fluid, BlockPos pos, double x, double y, double z,
        ...
        TextureAtlasSprite still = mc.getTextureMapBlocks().getAtlasSprite(fluid.getFluid().getStill(fluid).toString());
        TextureAtlasSprite flow = mc.getTextureMapBlocks().getAtlasSprite(fluid.getFluid().getFlowing(fluid).toString());
    ...
    public static void putTexturedQuad(
            ...
            EnumFacing face, int color, int brightness, boolean flowing) {
        putTexturedQuad(renderer, sprite, x, y, z, w, h, d, face, color, brightness, flowing, false);
    }
    public static void putTexturedQuad(
            BufferBuilder renderer, TextureAtlasSprite  sprite, double x, double y, double z, double w, double h, double d,
            EnumFacing face, int color, int brightness, boolean flowing, boolean flipHorizontal
    ) {
        ...
        int g = color >> 8 & 0xFF;
        int b = color & 0xFF;
        putTexturedQuad(renderer, sprite, x, y, z, w, h, d, face, r, g, b, a, l1, l2, flowing, flipHorizontal);
    }

    public static void putTexturedQuad(
            BufferBuilder renderer, TextureAtlasSprite sprite, double x, double y, double z, double w, double h, double d,
            EnumFacing face, int r, int g, int b, int a, int light1, int light2, boolean flowing) {
        putTexturedQuad(renderer, sprite, x, y, z, w, h, d, face, r, g, b, a, light1, light2, flowing, false);
    }

    public static void putTexturedQuad(
            BufferBuilder renderer, TextureAtlasSprite sprite, double x, double y, double z, double w, double h, double d,
            EnumFacing face, int r, int g, int b, int a, int light1, int light2, boolean flowing, boolean flipHorizontal
    ) {
        double minU;
        double maxU;
        double minV;
    ...
    public static void renderFluidSurface(
        ...
        renderer.pos(x2, y3, z2).color(r, g, b, a).tex(maxU, maxV).lightmap(light1, light2).endVertex();
        renderer.pos(x2, y4, z1).color(r, g, b, a).tex(maxU, minV).lightmap(light1, light2).endVertex();
    }

    public static void prepare(double x, double y, double z) {
