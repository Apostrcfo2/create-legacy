    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote && CreateLegacy.proxy.getSide() == Side.CLIENT) {
            ...
            if (nbt.hasKey("FirstPos", Constants.NBT.TAG_INT_ARRAY)) {
                ...
                File result = new File(downloads, "scene-" + System.nanoTime() + ".nbt");
                try {
                    ...
                    if (result.createNewFile()) {
                        ...
                        int minY = Math.min(firstPos.getY(), pos.getY());
                        int minZ = Math.min(firstPos.getZ(), pos.getZ());
                        int maxX = Math.max(firstPos.getX(), pos.getX());
                        int maxY = Math.max(firstPos.getY(), pos.getY());
                        int maxZ = Math.max(firstPos.getZ(), pos.getZ());
                        ...
                        player.sendStatusMessage(new TextComponentString("Saved scene to downloads folder"), true);
